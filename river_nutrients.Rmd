...
```{r}
# Read in the 2013 nutrient pollution from fertilizer dataset
...
# Per the methods, these values are log(x+1) transformed. We'd like to undo this...

# Double check to see if the values make sense...
# Log is the natural log. We want the log10 inverse
max <- exp(maxValue(nut_raw))

max10 <- exp(nut_raw, 10) + 10


# We can un-log these values:
nut_exp <- 10^(nut_raw) +1
...
ocean_reaches_pac <- as.data.frame(read_sf(here("data", "ocean_reaches"), layer = "shoreline_selectt"))
...
# For testing purposes, let's take the max of this to test the plume model on a single point:
which.max(snap_n_a$al_tn) #row 803

## FOR loop starts here, probably:
###### 
test_pt <- snap_n_a[803,]

# Set constants:
c <- 0.5
b <- 0.65
         
# Calculate the plume area
p_area <- c*(test_pt$CumAreaKM2^b)

pt_tn <- test_pt %>% 
  dplyr::select("al_tn", "geometry") %>% 
  st_as_sf()

# Rasterize that point
pt_raster <- rasterize(pt_tn, ocean, field = 1, background = NA)

pt_mx <- as.vector(pt_raster)
# Find which cells are equal to 1:
valid <- match(1, pt_mx)
# Find the neighboring cells
ngb <- adjacent(pt_raster, cells = valid, directions = 4, pairs = FALSE)
# Make them 1:
pt_raster[ngb] <- 1
pt_raster = pt_raster + ocean
hist(pt_raster)

# Then we expand that point in a circle using the adjacent function:

# That didn't work. The model returns a 1....


a <- adjacent(pt_raster)
